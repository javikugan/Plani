...
def main():
    ...
    for c in crate:
        crate_locations[c] = random.choice(locs_for_assignment)
    for p in person:
        person_locations[p] = random.choice(locs_for_assignment)
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        # All drones at the depot
        for x in drone:
            f.write(f"\t(at {x} deposito)\n")

        # All crates at the depot
        for c in crate:
            f.write(f"\t(at {c} {crate_locations[c]})\n")
        
        # Relate each crate to its content
        for idx, crates in enumerate(crates_with_contents):
            for c in crates:
                f.write(f"\t(contenido-caja {c} {content_types[idx]})\n")

        # All persons at the depot (you can change this if your domain requires)
        for p in person:
            f.write(f"\t(at {p} {person_locations[p]})\n")

        # You can add other initial predicates here according to your domain
        for x in range(options.persons):
        ...
        # Todos los drones deben terminar en el deposito
        for x in drone:
            f.write(f"\t(at {x} deposito)\n")

        # Each person should receive a crate with the content they need
        for x in range(options.persons):
            for y in range(len(content_types)):
                if need[x][y]:
                    person_name = person[x]
                    content_name = content_types[y]
                    # Goal: the person has a crate with that content
